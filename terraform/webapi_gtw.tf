# # # ----------------------------------------------------------------------------------------------------
# # # Rest API Gateway - using GET, POST, PUT, DELETE - Jokes
# # # ----------------------------------------------------------------------------------------------------
...
# # ----------------------------------------------------------------------------------------------------
# # Rest API Gateway - usieng GET - HCA
# # ----------------------------------------------------------------------------------------------------

# Define the API Gateway REST API with regional endpoint type
resource "aws_api_gateway_rest_api" "hca_api" {
  name        = "${var.name_prefix}-${var.rest_api_name}"
  description = var.rest_api_description
...
# # ----------------------------------------------------------------------------------------------------
# # Define Resources- HCA
# # ----------------------------------------------------------------------------------------------------

# Create the resource for 'hca' path
resource "aws_api_gateway_resource" "hca_resource" {
  rest_api_id = aws_api_gateway_rest_api.hca_api.id
  parent_id   = aws_api_gateway_rest_api.hca_api.root_resource_id
  path_part   = "hca"
}

# # ----------------------------------------------------------------------------------------------------
# # Lambda permission- HCA
# # ----------------------------------------------------------------------------------------------------
...
resource "aws_lambda_permission" "api_gateway_permission" {
  statement_id  = "AllowAPIGatewayInvoke"
  action        = "lambda:InvokeFunction"
  function_name = aws_lambda_function.hca_lambda.function_name
  principal     = "apigateway.amazonaws.com"
  source_arn    = "${aws_api_gateway_rest_api.hca_api.execution_arn}/*/*"
}
...
# Define the GET method
resource "aws_api_gateway_method" "get_hca" {
  rest_api_id   = aws_api_gateway_rest_api.hca_api.id
  resource_id   = aws_api_gateway_resource.hca_resource.id
  http_method   = "GET"
  authorization = "NONE"
}
...
# # ----------------------------------------------------------------------------------------------------
# # Add Methods response - /hca
# # ----------------------------------------------------------------------------------------------------

resource "aws_api_gateway_method_response" "get_method_response_200" {
  rest_api_id = aws_api_gateway_rest_api.hca_api.id
  resource_id = aws_api_gateway_resource.hca_resource.id
  http_method = aws_api_gateway_method.get_hca.http_method
  status_code = "200"
...
# # ----------------------------------------------------------------------------------------------------
# # Setup up Integration - \hca
# # ----------------------------------------------------------------------------------------------------

# Integrate GET method with Lambda
resource "aws_api_gateway_integration" "get_hca_integration" {
  rest_api_id             = aws_api_gateway_rest_api.hca_api.id
  resource_id             = aws_api_gateway_resource.hca_resource.id
  http_method             = aws_api_gateway_method.get_hca.http_method
  integration_http_method = "POST"
  type                    = "AWS"
  uri                     = aws_lambda_function.hca_lambda.invoke_arn
...
# # ----------------------------------------------------------------------------------------------------
# # Define Integration responses - \hca
# # ----------------------------------------------------------------------------------------------------

resource "aws_api_gateway_integration_response" "get_hca_integration_response" {

  depends_on = [
    aws_api_gateway_integration.get_hca_integration,
    aws_api_gateway_method_response.get_method_response_200
  ]
...
# # ----------------------------------------------------------------------------------------------------
# # Deployment the API- HCA
# # ----------------------------------------------------------------------------------------------------

# Deployment for the API Gateway
resource "aws_api_gateway_deployment" "hca_deployment" {
  depends_on = [
    aws_api_gateway_resource.hca_resource,
    aws_api_gateway_method.get_hca,
    aws_api_gateway_method.options_cors,
    aws_api_gateway_method_response.get_method_response_200,
    aws_api_gateway_method_response.option_cors_method_response_200,
    aws_api_gateway_integration.get_hca_integration,
    aws_api_gateway_integration.options_cors_integration,
    aws_api_gateway_integration_response.get_hca_integration_response,
    aws_api_gateway_integration_response.option_cors_integration_response,
  ]
...
resource "aws_api_gateway_stage" "api_stage" {
  rest_api_id   = aws_api_gateway_rest_api.hca_api.id
  deployment_id = aws_api_gateway_deployment.hca_deployment.id
  stage_name    = var.stage_name
  description   = var.stage_name_desc
}
...
# This Terraform configuration establishes a comprehensive REST API Gateway infrastructure for a HCA service with GET Method capabilities:  
#
# Architecture Overview - Creates a Regional REST API Gateway with two paths:
#     /hca for bulk operations
#
# Core Functionality:
# - READ: Retrieves jokes via GET
#
...
# The infrastructure provides complete API management for a scalable hca service using Lambda and DynamoDB.
